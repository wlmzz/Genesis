#!/usr/bin/env python3
"""
Genesis - ULTIMATE FULL BODY MAPPING
Tracking perfetto del corpo intero con massima precisione

MAPPATURA COMPLETA:
- CORPO: 33 landmarks MediaPipe Pose (scheletro completo)
- VOLTO: 478 landmarks MediaPipe Face (multi-angolo)
- MANI: 21 landmarks per mano (tutte le dita)
- LIP READING: Lettura labiale e riconoscimento parole
- OGGETTI: 80 classi COCO + rilevamento in mano

TOTALE: 33 + 478 + 42 + lip reading = 553+ landmarks del corpo!
"""
import cv2
import numpy as np
...
# MediaPipe Pose connections (scheletro completo)
POSE_CONNECTIONS = [
    # Torso
...
def draw_pose_landmarks(frame, pose_landmarks):
    """Disegna 33 landmark del corpo con scheletro completo"""
    h, w, _ = frame.shape
    ...
    for connection in POSE_CONNECTIONS:
        ...
        if start_idx < len(points) and end_idx < len(points):
            if points[start_idx][2] > 0.5 and points[end_idx][2] > 0.5:
                ...
                # Colore CIANO per corpo
                cv2.line(frame, start_point, end_point, (255, 255, 0), 3)

    # Disegna landmark points
    for i, (x, y, visibility) in enumerate(points):
        if visibility > 0.5:
            # Punti principali più grandi
            if i in [0, 11, 12, 13, 14, 15, 16, 23, 24, 25, 26]:  # Keypoints importanti
                cv2.circle(frame, (x, y), 8, (0, 255, 255), -1)
                cv2.circle(frame, (x, y), 9, (255, 255, 0), 2)
            else:
                cv2.circle(frame, (x, y), 5, (255, 255, 0), -1)

    # Bounding box attorno al corpo
    if points:
        visible_points = [(x, y) for x, y, v in points if v > 0.5]
        if visible_points:
            ...
            # Box CIANO per corpo
            cv2.rectangle(frame, (x1-10, y1-10), (x2+10, y2+10), (255, 255, 0), 3)
            cv2.putText(frame, "CORPO (33 landmarks)", (x1-10, y1-25),
...
def main():
    print("=" * 80)
    print("Genesis - ULTIMATE FULL BODY MAPPING")
    print("=" * 80)
    print("Tracking PERFETTO del corpo intero:")
    print("  - CORPO: 33 landmarks MediaPipe Pose (scheletro completo 3D)")
    print("  - VOLTO: 478 landmarks MediaPipe Face (multi-angolo)")
    ...
    print()
    print("TOTALE: 553+ landmark points del corpo umano!")
    print()
    print("Controlli:")
    print("  - ESC: Esci")
    print("  - 'p': Toggle Pose (corpo)")
    print("  - 'f': Toggle Face")
    ...
    if not os.path.exists(pose_model_path):
        ...
        print(f"Downloading pose model (heavy - max precision)...")
        urllib.request.urlretrieve(url, pose_model_path)
        print(f"✓ Pose model downloaded")
    else:
        print(f"✓ Pose model exists")
    ...
    # MediaPipe Pose Landmarker (33 punti corpo)
    ...
    print("\n✓ Sistema ULTIMATE attivo! Cerca finestra 'Genesis - Ultimate Body Mapping'\n")
    ...
    while True:
        ...
        # Converti frame per MediaPipe (una sola volta)
        rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        mp_image = mp.Image(image_format=mp.ImageFormat.SRGB, data=rgb_frame)

        # === POSE LANDMARKS (33 punti corpo) ===
        if show_pose:
            pose_result = pose_detector.detect(mp_image)

            if pose_result.pose_landmarks:
                for pose_landmarks in pose_result.pose_landmarks:
                    draw_pose_landmarks(display_frame, pose_landmarks)
        ...
        # Status info (sinistra alto)
        status_y = 30
        cv2.putText(display_frame, f"Pose: {'ON' if show_pose else 'OFF'} (33 body landmarks)", (10, status_y),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        status_y += 30
        cv2.putText(display_frame, f"Face: {'ON' if show_face else 'OFF'} (478 pts)", (10, status_y),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        status_y += 30
        ...
        cv2.imshow("Genesis - Ultimate Body Mapping", display_frame)
    ...
    print("\n✓ Ultimate Body Mapping terminato")
